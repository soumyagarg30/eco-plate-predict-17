                ...
                {filteredRestaurants.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredRestaurants.map((restaurant, index) => (
                      <Card key={restaurant.id} className="overflow-hidden hover:shadow-xl transition-all duration-300 border-0 shadow-lg bg-white flex flex-col h-full">
                        {/* Restaurant Image */}
                        ...
                        <CardContent className="p-4 flex flex-col flex-1">
                          ...
                          <div className="mb-4 flex-1">
                            <div className="flex justify-between items-center mb-2">
                              <h4 className="font-semibold text-gray-900 text-xs">Menu Highlights</h4>
                              {restaurant.menuItems.length > 2 && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => toggleRestaurantExpansion(restaurant.id)}
                                  className="text-xs h-6 px-2 text-green-600 hover:text-green-700 hover:bg-green-50"
                                >
                            ...
                            {restaurant.menuItems.length > 0 ? (
                              <div className="space-y-1.5 max-h-32 overflow-y-auto">
                                {restaurant.menuItems
                                  ...
                                  .map((item) => (
                                  <div key={item.id} className="border border-gray-100 rounded-md p-2 bg-gray-50/50">
                                    <div className="flex justify-between items-start gap-2">
                                      <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                                          <span className="font-medium text-gray-900 text-xs line-clamp-1">{item.name}</span>
                                          <div className="flex gap-1">
                                            {item.is_vegetarian && (
                                              <Badge variant="secondary" className="text-xs px-1 py-0 bg-green-100 text-green-700 h-4 text-[10px]">
                                                Veg
                                              </Badge>
                                            )}
                                            {item.is_vegan && (
                                        ...
                                        {item.description && (
                                          <p className="text-gray-500 text-[10px] line-clamp-1">{item.description}</p>
                                        )}
                                      </div>
                                      <span className="font-bold text-green-600 text-xs flex-shrink-0">${item.price.toFixed(2)}</span>
                                    </div>
                                  </div>
                                ))}
                              </div>
                            ) : (
                              <p className="text-xs text-gray-400 italic text-center py-2 bg-gray-50/50 rounded-md border border-gray-100">
                                Menu coming soon...
                              </p>
                            )}
                          </div>
                          ...
                          <div className="flex gap-2 mt-auto">
                            <Button 
                              ...
                              variant="outline"
                              size="sm"
                              className="flex-1 border-gray-200 hover:bg-gray-50 text-xs h-8"
                            >
                              View Details
                            </Button>
                            <Button 
